...
export class Transaction {
    ...
    connect(type: ProviderEnums.Provider): void {
        ...
        if (this._prevType !== this._currentType) {
            ...
            this._prevType = type
            console.log('» [Transaction]Connect new provider:  %c' + this._currentType, 'color: #FABB51; font-size:14px')
        } else {
            console.log('» [Transaction]Continue to connect the current provider:  %c' + this._prevType, 'color: #FABB51; font-size:14px')
        }
    }
