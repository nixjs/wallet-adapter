...
export class Transaction {
    ...
    connect(type: ProviderEnums.Provider): void {
        ...
        if (this.#prevType !== this.#currentType) {
            ...
            this.#prevType = type
            console.log('» [Transaction]Connect new provider:  %c' + this.#currentType, 'color: #FABB51; font-size:14px')
        } else {
            console.log('» [Transaction]Continue to connect the current provider:  %c' + this.#prevType, 'color: #FABB51; font-size:14px')
        }
    }
