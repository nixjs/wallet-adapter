...
export class Provider {
  ...
  > {
    if (from.address) {
      const coins = (await this.query.getOwnedCoins(from.address)).filter(
  ...
  async transferObject(
    ...
    const object = (await this.query.getOwnedObjects(vault.address())).find(
...
export class QueryProvider {
  ...
  public async getGasObject(
    address: string,
    gasBudget: number
  ): Promise<SUIUtil.CoinObject | undefined> {
  ...
  ): Promise<TransactionTypes.TransactionObject[]> {
    if (!objectRefs) {
      return [];
    }
...
export class TxProvider {
  ...
  > {
    try {
      if (!from.address || !from.publicKeyHex)
        throw new Error("Owner info not found");
      const actualAmount = BigInt(amount + DEFAULT_GAS_BUDGET_FOR_TRANSFER_SUI);
      const objects = coins.map((coin) => coin.object);
      ...
      const gasCostForPay = SUIUtil.Coin.estimatedGasCostForPay(coins.length);
      let inputCoins = await SUIUtil.Coin.assertAndGetSufficientCoins(
        objects,
        BigInt(amount),
        gasCostForPay
      );
      ...
      if (inputCoins.length === coins.length) {
        // We need to pay for an additional `transferSui` transaction now, assert that we have sufficient balance
        // to cover the additional cost
        await SUIUtil.Coin.assertAndGetSufficientCoins(
          objects,
          BigInt(amount),
          gasCostForPay + DEFAULT_GAS_BUDGET_FOR_TRANSFER_SUI
        );
