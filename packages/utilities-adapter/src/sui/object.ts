...
import { BaseCoinType, BaseCoinTypeRegex, BaseMaxGasAmount } from './const'
...
export class Coin {
    public static isCoin(obj: SuiMoveObject) {
        return obj.type.startsWith(BaseCoinType)
    }
    ...
    static getCoinTypeArg(obj: SuiMoveObject) {
        const res = obj.type.match(BaseCoinTypeRegex)
    ...
    static getCoinTypeFromArg(coinTypeArg: string) {
        return `${BaseCoinType}<${coinTypeArg}>`
    }

    static estimatedGasCostForPay(numInputCoins: number): number {
        return BaseMaxGasAmount * Math.max(2, Math.min(100, numInputCoins / 2))
    }

    static async assertAndGetSufficientCoins(coins: SuiMoveObject[], amount: bigint, gasBudget?: number) {
        const inputCoins = await CoinAPI.selectCoinSetWithCombinedBalanceGreaterThanOrEqual(coins, amount + BigInt(gasBudget ?? 0))
...
export class Nft {
    ...
    public static getNftObject(obj: SuiMoveObject, previousTransaction?: string): NftObject {
        return {
            ...
            objectType: obj.type,
            fields: obj.fields,
            hasPublicTransfer: obj.has_public_transfer ? obj.has_public_transfer : false,
        }
    }
}
