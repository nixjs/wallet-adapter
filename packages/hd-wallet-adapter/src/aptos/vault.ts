...
export class AptosVault extends BaseProvider {
    ...
    async getAccountFromMnemonic(derivationPath: number, mnemonic: string): Promise<VaultTypes.AccountObject & { path: string }> {
        const path = Crypto.derivationHdPath(AptosWalletConfig.CoinType, derivationPath)
        const ac = await AptosVault.fromDerivePath(Crypto.derivationHdPath(AptosWalletConfig.CoinType, derivationPath), mnemonic)
